...
var _svg4everybody = __webpack_require__(328);

var _svg4everybody2 = _interopRequireDefault(_svg4everybody);

var _svgUse = __webpack_require__(329);
...
var _Datapicker = __webpack_require__(333);

var _Datapicker2 = _interopRequireDefault(_Datapicker);

var _Validation = __webpack_require__(335);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    App.Input = new _Input2.default();
    App.Datapicker = new _Datapicker2.default();
    App.Validation = new _Validation2.default();
    App.Modal = new _Modal2.default();
});
    ...
    _createClass(SvgUse, [{
        ...
        value: function init() {
            ...
            (function (window, document) {
                ...
                // if (!App.debug) file = window.SITE_TEMPLATE_PATH + '/icons/svg/symbols.svg';

                // if (window.location.href.indexOf('3000') !== -1) {
                //     file = 'local/templates/nkrt74_de/icons/svg/symbols.svg';
                // }
    ...
    _createClass(Select, [{
        ...
        value: function init() {
            ...
            this.select.forEach(function (element) {
                ...
                element.addEventListener('click', _this.selectOption.bind(_this));
            });
        }
    }, {
        key: 'handleClick',
        value: function handleClick(event) {
            ...
            var dropdown = this.findElement(event, 'select__options');

            if (!target.classList.contains('select__option')) {
                var filterDropdown = [].concat(_toConsumableArray(this.dropdown)).filter(function (element) {
                    return element.classList.contains('visible');
                });
                filterDropdown.forEach(function (options) {
                    return _this2.closeOptions(options);
                });
            }

            dropdown.classList.contains('visible') ? this.closeOptions(dropdown) : this.openOptions(dropdown);
        }
    }, {
        key: 'selectOption',
        value: function selectOption(event) {
            var dropdown = this.findElement(event, 'select__options');
            var input = this.findElement(event, 'select__input');
            var text = this.findElement(event, 'select__text');
            var label = this.findElement(event, 'select__label');
            var target = event.target;


            if (!dropdown.classList.contains('visible') && target.classList.contains('select__option')) {
        ...
        key: 'openOptions',
        value: function openOptions(childElement) {
            return childElement.classList.add('visible');
        }
    }, {
        key: 'closeOptions',
        value: function closeOptions(childElement) {
        ...
        value: function closeDropdown() {
            ...
            this.dropdown.forEach(function (options) {
                return _this3.closeOptions(options);
            });
        }
    }, {
        key: 'findElement',
        value: function findElement(event, selector) {
            event.stopPropagation();
            var parent = event.target === 'div.select' ? event.target : event.target.closest('div.select');
            var childs = parent.children;
            var arr = [].concat(_toConsumableArray(childs)).filter(function (childElement) {
    ...
    _createClass(Input, [{
        ...
        value: function init() {
            ...
            this.input.forEach(function (element) {
                element.addEventListener('input', _this.handleInput.bind(_this));
                element.addEventListener('click', _this.handleClick.bind(_this));
            });
        }
    }, {
        key: 'handleClick',
        value: function handleClick(event) {
            var input = this.findElement(event, 'input__input');
        ...
        value: function handleInput(event) {
            var input = this.findElement(event, 'input__input');
            var label = this.findElement(event, 'input__label');

            var inputValue = input.value.trim();

            !!inputValue ? label.classList.add('input__label--small') : label.classList.remove('input__label--small');
        }
    }, {
        key: 'findElement',
        value: function findElement(event, selector) {
            var parent = event.target === 'div.input' ? event.target : event.target.closest('div.input');
            var childs = parent.children;
            console.log(parent);
            var arr = [].concat(_toConsumableArray(childs)).filter(function (childElement) {
...
var _flatpickr = __webpack_require__(334);
...
var Datapicker = function () {
    ...
    _createClass(Datapicker, [{
        key: "init",
        value: function init() {
            var fp = (0, _flatpickr2.default)(this.datapicker, this.config);
        }
    }]);
...
var Validation = function () {
    ...
    _createClass(Validation, [{
        ...
        value: function handleClick(event) {
            ...
            this.formElements.forEach(function (element) {
                ...
                if (element.dataset.type === 'email') {
                    val = _this2.validEmail(element.value);
                }

                _this2.model.push(!!val);
            });

            var filterInvalid = this.model.filter(function (el) {
                return el === false;
            });

            this.model.forEach(function (el, index) {
                if (_this2.formElements[index].dataset.type === 'email' && !el) {
                    return _this2.addErrorMessage(_this2.formElements[index]), _this2.renderError(index);
                } else {

                    return el === false && _this2.renderError(index);
                }
            });
        ...
        key: 'renderError',
        value: function renderError(index) {
            var error = this.formElements[index].querySelector('.icon-error');

            error === null && this.formElements[index].insertAdjacentHTML('beforebegin', '<svg class="icon icon-error">\n            <use xlink:href="#icon-error"></use>\n        </svg>');
        }
    }, {
        key: 'renderSucsess',
        value: function renderSucsess(event) {
            var success = event.target.parentNode.querySelector('.icon-success');
            var element = this.findElement(event, 'input__input');
            success === null && element.insertAdjacentHTML('beforebegin', '<svg class="icon icon-success">\n            <use xlink:href="#icon-success"></use>\n        </svg>');
        }
    }, {
        key: 'removeSucsess',
        value: function removeSucsess(event) {
            var success = event.target.parentNode.querySelector('.icon-success');
        ...
        key: 'removeError',
        value: function removeError(event) {
            var error = event.target.parentNode.querySelector('.icon-error');
        ...
        key: 'findElement',
        value: function findElement(event, selector) {
            var parent = event.target === 'div.validation' ? event.target : event.target.closest('div.validation');
            var childs = parent.children;
            var arr = [].concat(_toConsumableArray(childs)).filter(function (childElement) {
                return childElement.classList.contains(selector);
            });
        ...
        key: 'validEmail',
        value: function validEmail(el) {
            var filter = /^\s*[\w\-\+_]+(\.[\w\-\+_]+)*\@[\w\-\+_]+\.[\w\-\+_]+(\.[\w\-\+_]+)*\s*$/;
            return String(el).search(filter) != -1;
        }
    }, {
        key: 'addErrorMessage',
        value: function addErrorMessage(el) {
            var parent = el.parentNode;
            var errorMessage = parent.querySelector('.error-message');

            errorMessage === null && parent.insertAdjacentHTML('beforeend', '<div class=\'error-message form__error-message\'>Invalid format</div>');
        }
    }, {
